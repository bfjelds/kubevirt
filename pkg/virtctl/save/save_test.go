...
var _ = Describe("Saving", func() {
	...
	DescribeTable("should save VMI", func(persistOptions *v1.PersistOptions) {
		...
		vmiInterface.EXPECT().Persist(context.Background(), vmi.Name, persistOptions).Return(nil).Times(1)

		var command *cobra.Command
		if len(pauseOptions.DryRun) == 0 {
			command = clientcmd.NewVirtctlCommand(pause.COMMAND_SAVE, "vmi", vmName)
		} else {
			command = clientcmd.NewVirtctlCommand(pause.COMMAND_SAVE, "--dry-run", "vmi", vmName)
		}
	...
	DescribeTable("should save VM", func(persistOptions *v1.PersistOptions) {
		...
		vmiInterface.EXPECT().Save(context.Background(), vm.Name, persistOptions).Return(nil).Times(1)

		var command *cobra.Command
		if len(pauseOptions.DryRun) == 0 {
			command = clientcmd.NewVirtctlCommand(pause.COMMAND_SAVE, "vm", vmName)
		} else {
			command = clientcmd.NewVirtctlCommand(pause.COMMAND_SAVE, "--dry-run", "vm", vmName)
		}
		Expect(command.Execute()).To(Succeed())
	},
		Entry("", &v1.PersistOptions{}),
