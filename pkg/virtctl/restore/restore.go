...
func NewRestoreCommand(clientConfig clientcmd.ClientConfig) *cobra.Command {
 ...
 Second argument is the name of the resource. Third argument is the location to restore the resource.`,
...
func (vc *VirtCommand) Run(args []string) error {
	...
	case COMMAND_RESTORE:
		...
		case ARG_VM_LONG, ARG_VM_SHORT:
			...
			vmiName := vm.Name
			err = virtClient.VirtualMachineInstance(namespace).Restore(context.Background(), vmiName, &kubevirtV1.PersistOptions{PersistPath: persistPath, DryRun: dryRunOption})
			if err != nil {
		...
		case ARG_VMI_LONG, ARG_VMI_SHORT:
			err = virtClient.VirtualMachineInstance(namespace).Restore(context.Background(), resourceName, &kubevirtV1.PersistOptions{PersistPath: persistPath, DryRun: dryRunOption})
			if err != nil {
